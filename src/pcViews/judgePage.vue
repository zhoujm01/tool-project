                    ...
                    <div class="contentItemArea" @click="clickContentCard(item2)">
                      <div class="contentItemBackground" v-if="item2 ? item2.bgType ? true : false : false">
                        ...
                        <div style="width: 100%;height:100%;" v-if="item2.bgType == 1">
                          <img :src="item2.bgimg" style="width: 100%;height:100%;" />
                        </div>
                        <!-- 视频 -->
                        <div style="width: 100%;height:100%;" v-if="item2.bgType == 2">
                          <video style="width: 100%;height:100%;" :src="item2.bgimg" :autoplay="false"></video>
                          <PlayCircleFilled style="font-size: 70px;color: rgb(0, 190, 201);position: absolute;background: white;
                          border-radius: 25px;left: calc(50% - 35px);top: calc(50% - 35px);" />
                        </div>
                      </div>
                      <!-- 空白图片 -->
                      ...
                      <div class="contentItemTextArea" :id="'contentItemTextAreaId' + index">
                        <a-typography class="contentItemText" :id="'contentItemTextId' + index"
                          v-if="item2 ? item2.contenttext!=null ? true : false : false">
                        ...
                        <div class="showMore" v-if="item2 ? item2.moreflag ? item2.moreflag : false : false">
                          <div>查看更多</div>
                        </div>
                      </div>
                      <div class="contentItemTitleArea" v-if="item2 ? item2.title ? true : false : false">
            ...
            <a-form :model="formState" v-if="state.drawerAdd == false" name="basic" :layout="'vertical'"
              ...
              <a-form-item label="类别" name="level" :rules="[{ required: true, message: '请选择类别！' }]">
                <a-cascader v-model:value="formState.level" :options="selectList" placeholder="请选择类别"
                  style="text-align: left;" :allowClear="false"></a-cascader>
              </a-form-item>

              <a-form-item label="描述" name="content">
                <a-textarea v-model:value="formState.content" placeholder="请输入描述"
            ...
            <a-form :model="formState" v-if="state.drawerAdd == true" name="basic" :layout="'vertical'" autocomplete="off"
              ...
              <a-form-item label="名称" name="mainName" :rules="[{ required: true, message: '请输入类别！' }]">
                <a-select v-model:value="formState.mainName" style="width: 100%;text-align:left;" placeholder="请输入类别"
                  :options="selectList" :filter-option="filterOneOption" show-search allowClear notFoundContent
                  :showArrow="false" @select="selectOneMenu"></a-select>
              </a-form-item>
              <a-form-item label="图标" name="content" :rules="[{ required: true, message: '请选择图标！' }]">
                <a-radio-group v-model:value="formState.mainIcon" name="radioGroup"
                  style="width:95%;display:flex;align-items:center;">
                  <div @click="() => { formState.mainIcon = 'mobileIcon1' }" class="radioArea">
                    <img :src="mobileIcon1" class="iconImg" />
                    <a-radio value="mobileIcon1" style="margin-right:0px"></a-radio>
                  </div>
                  <div @click="() => { formState.mainIcon = 'mobileIcon2' }" class="radioArea">
                    <img :src="mobileIcon2" class="iconImg" />
                    <a-radio value="mobileIcon2" style="margin-right:0px"></a-radio>
                  </div>
                  <div @click="() => { formState.mainIcon = 'mobileIcon3' }" class="radioArea">
                    <img :src="mobileIcon3" class="iconImg" />
                    <a-radio value="mobileIcon3" style="margin-right:0px"></a-radio>
                  </div>
                  <div @click="() => { formState.mainIcon = 'mobileIcon4' }" class="radioArea">
                    <img :src="mobileIcon4" class="iconImg" />
                    <a-radio value="mobileIcon4" style="margin-right:0px"></a-radio>
                  </div>
                  <div @click="() => { formState.mainIcon = 'mobileIcon5' }" class="radioArea">
                    <img :src="mobileIcon5" class="iconImg" />
              ...
              <a-form-item label="名称" name="subName" :rules="[{ required: true, message: '请输入子类别！' }]">
                <a-select v-model:value="formState.subName" style="width: 100%;text-align:left;" placeholder="请输入子类别"
                  ...
                  :showArrow="false" @select="selectTwoMenu"></a-select>
  ...
  setup() {
    ...
    async function getparams() {
      await getReportData(null).then(res => {
        ...
        res.diagnosis.forEach((item) => {
          ...
          if (item.childMenuList != null && item.childMenuList.length > 0) {
            item.childMenuList.forEach((item2) => {
              if (item2.childContentList != null && item2.childContentList.length > 0) {
                item2.childContentList.forEach((item3) => {
                  ...
                  if (item3.medialist != null && item3.medialist.length > 0) {
                    ...
                    // 4级按key排序
                    let templist1 = [];
                    for (let j = 1; j <= item3.medialist.length; j++) {
                      let tempItem1 = item3.medialist.filter((item5) => {
                        let indexkey1 = item5.index.split("-");
                        return j == indexkey1[3]
                      })
                      if (tempItem1 != null && tempItem1.length > 0) {
                        templist1.push(tempItem1[0]);
                      }
                    }
                    item3.medialist = templist1;
      ...
      if (state.value.drawerAdd == false) {
        ...
        if (formState.title != null && formState.title != "") {
          ...
          if (formState.level != null && formState.level.length > 0) {
            ...
            if (state.value.drawerAdd == false) {
              // 编辑数据
              await changeReportData(state.value.currentContentData.key, "set", formData)
                .then(async res => {
                  if (res.code == 200) {
                    ...
                    // 删除所有4级媒体数据
                    if (
                      state.value.currentContentData.medialist != null &&
                      state.value.currentContentData.medialist.length > 0
                    ) {
                      let len1 = 0;
                      await state.value.currentContentData.medialist.forEach(async item => {
                        ...
                        await changeReportData(item.index, "delete", oldFour)
                          .then(async res2 => {
                            if (res2.code == 200) {
                              ...
                              if (len1 == state.value.currentContentData.medialist.length) {
                                // 删除4级成功
                                // 添加新的4级数据
                                if (formState.fileList != null && formState.fileList.length > 0) {
                                  let len2 = 0;
                                  formState.fileList.forEach(async (item3, index3) => {
                                    let newFour = new FormData();
                                    let parentKey1 = state.value.currentContentData.key;
                                    let cIndex = index3 + 1;
                                    let itemIndex = parentKey1 + "-" + cIndex;
                                    newFour.append("three", parentKey1);
                                    if (item3.type.indexOf("image") != -1) {
          ...
          if (formState.subName != null && formState.subName != "") {
            if (formState.title != null && formState.title != "") {
              ...
              if (formState.second != null && formState.second != "") {
                ...
                tempform1.append("icon", formState.mainIcon);
                tempform1.append("mobileicon", formState.mainIcon);
                tempform1.append("label", formState.mainName);
                tempform1.append("title", formState.mainName);

                await changeReportData(formState.first, "set", tempform1).then(
                  async res => {
                    if (res.code == 200) {
                      ...
                      await changeData("three", "select", temp).then(async res1 => {
                        if (res1.success == 0) {
                          let maxKey = 0;
                          // console.log("threekey", res1.data)
                          if (res1.data != null && res1.data.length > 0) {
                            res1.data.forEach(item1 => {
                              let twolist = item1.key.split("-");
                              let two = twolist[0] + "-" + twolist[1];
                              if (two == formState.second) {
                                let twokey = item1.key.split("-")[2];
                          ...
                          // 添加新的3级数据
                          let nowkey3 = newKey + 1;
                          let second = formState.second;
                          let key3 = second + "-" + nowkey3;
                          let tempform3 = new FormData();
                          // 新增三四级数据
                          addThreeAndFour(second, key3);
                        } else {
                          // 获取3级失败
                          // console.log("获取3级数据-失败:", res);
                          message.error("提交失败！")
                          refreshContentData();
                        }
                      }).catch((err) => {
                ...
                // 新增2级菜单
                if (formState.first != null && formState.first != "") {
                  // 选中已有一级菜单
                  // 已有一级菜单 // 新增二级菜单
                  // 修改一级图标
                  let tempform1 = new FormData();
                      ...
                      if (res.code == 200) {
                        ...
                        await changeData("second", "select", temp).then(async res1 => {
                          if (res1.success == 0) {
                            ...
                            if (res1.data != null && res1.data.length > 0) {
                              res1.data.forEach(item1 => {
                                if (item1.first == formState.first) {
                            ...
                            // 添加新的2级数据
                            let nowkey2 = newKey + 1;
                            let key2 = formState.first + "-" + nowkey2;
                            let tempform2 = new FormData();

                            tempform2.append("label", formState.subName);
                            tempform2.append("first", formState.first);
                            tempform2.append("title", formState.subName);
                            await changeReportData(key2, "insert", tempform2)
                              .then(async res => {
                                if (res.code == 200) {
                                  // console.log("新增2级数据-成功:", res);
                                  // 新增3级
                                  addThreeAndFour(key2, key2 + "-1");
                ...
                } else if (formState.first == null || formState.first == "") {
                  // 新增一级菜单 // 新增二级菜单
    ...
    const clickDelete = async () => {
      ...
      if (
        ...
        let len1 = 0;
        state.value.currentContentData.medialist.forEach(async item => {
          let oldFour = new FormData();
          oldFour.append("imgurl", item.imgurl);
          oldFour.append("type", item.type);
          oldFour.append("videourl", item.videourl);
            ...
            .then(async res2 => {
              if (res2.code == 200) {
                ...
                if (len1 == state.value.currentContentData.medialist.length) {
                  // 删除4级成功
                  // 开始删除3级
                  let oldThree = new FormData();
                  oldThree.append("second", state.value.currentContentData.second);

                  await changeReportData(state.value.currentContentData.key, "delete", oldThree)
                    .then(async res3 => {
                      if (res3.code == 200) {
                        // console.log("删除3级数据-成功:", res3);
                        message.success("删除成功");
                        await refreshContentData();
                        clickContentBack();
                      } else {
                        // console.log("删除3级数据-失败:", res3);
                        message.error("删除失败！");
                      }
                    }).catch((err) => {
                      // console.log("删除3级数据-失败:", err);
                      message.error("删除失败！");
                    })
                }
              } else {
...
.leftArea {
  ...
  overflow-y: auto;
  overflow-x: hidden;
  background: url("/picture/pc/bg_架线.png");
